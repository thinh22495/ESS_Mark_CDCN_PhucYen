...
Imports ESS.Objects
Public Class frmESS_BienLaiThuThiLai
...
#Region "Functions And Subs"
    Public Overloads Sub ShowDialog(ByVal idx_bl As Integer, ByVal clsBL As BienLaiThu_Bussines, ByVal ID_he As Integer, ByVal ID_lops As String, ByVal Hoc_ky As Integer, ByVal Nam_hoc As String, ByVal Dot_thu As Integer, ByVal Lan_thu As Integer, ByVal ID_thu_chi As Integer)
        midx_bl = idx_bl
        mclsBL = clsBL
        mID_he = ID_he
        mID_lops = ID_lops
    ...
    Private Sub LoadThongTinTaiChinhSinhVien(ByVal ID_sv As Integer)
        ...
        If clsDanhSach.Count > 0 Then
            ...
            Dim objHoSo_Bll As New ThongTinSinhVien_Bussines(objDanhSach.ID_sv)
            mID_he = objHoSo_Bll.ESSThongTinSinhVien.ID_he
            labTen_lop.Text = objDanhSach.Ten_lop & " (" & objHoSo_Bll.ESSThongTinSinhVien.Ten_he & " - K" & objHoSo_Bll.ESSThongTinSinhVien.Khoa_hoc & ")"
            txtMa_sv.Text = objDanhSach.Ma_sv
            Dim clsDiem As New BangDiemCaNhan_Bussines("P1", ID_sv, objHoSo_Bll.ESSThongTinSinhVien.ID_dt, 0, "", True) 'Load bang diem sinh vien
            ...
            If ID_bien_lai > 0 Then
                ...
                Dim dtMucThiLai As DataTable = mclsBL.HienThi_ESSMucThiLai_DanhSach(cmbHoc_ky.SelectedValue, cmbNam_hoc.Text, mID_he)
                ...
                Else
                    ...
                    dtMonThu = mclsBL.DanhSachHocPhanBienLaiThuThiLai(dtMucThiLai, Ngoai_ngan_sach, cmbHoc_ky.SelectedValue, cmbNam_hoc.Text, 0, ID_sv, cmbID_thu_chi.SelectedValue, 0, CInt(cmbLan_thu.Text))
                    'Xoá các học phần đã tích luỹ
                    XoaHocPhanTichLuy(dtDiem, dtMonThu)
                End If
                ...
                Dim dtTongHop As DataTable = mclsBL.TongHop_HocPhi(ID_sv, Ngoai_ngan_sach, mID_he)
            ...
            Else    'Viết biên lai mới thì loại những Học phần dã thu
                ...
                Dim dtMonThu As DataTable = mclsBL.DanhSachHocPhanBienLaiThuThiLai(dtMucThiLai, Ngoai_ngan_sach, cmbHoc_ky.SelectedValue, cmbNam_hoc.Text, 0, ID_sv, cmbID_thu_chi.SelectedValue, midx_bl, CInt(cmbLan_thu.Text))
                'Xoá các học phần đã tích luỹ
                XoaHocPhanTichLuy(dtDiem, dtMonThu)
                Me.grdMonThu.DataSource = dtMonThu.DefaultView
                Dim so_tien_phai_nop As Integer = 0
    ...
    Private Sub XoaHocPhanTichLuy(ByVal dtDiem As DataTable, ByRef dtMonThu As DataTable)
        Try
            For i As Integer = dtMonThu.Rows.Count - 1 To 0 Step -1
                ...
                Else
                    If CType(dv(0).Item("TBCMH1"), Double) > 4 Then
                        dtMonThu.Rows(i).Delete()
                        dtMonThu.AcceptChanges()
                    End If
                End If
