...
Public Class frmESS_DMXepHangNamDaoTao
    Dim add As Boolean = False
    Dim edit As Boolean = False
    Dim loaded As Boolean = False
#Region "Khai bao Bien & Hang"
    Private Const Header As String = "XẾP HẠNG NĂM ĐÀO TẠO"
    Private Const TableName As String = "MARK_XepHangNamDaoTao_TC_09"
    Private Const FieldID As String = "ID_xep_hang"
    Private Const FieldMa As String = "Nam_thu"
    Private Const FieldMaEn As String = "Nam_thu_en"
    Private Const FieldTuDiem As String = "Tu_tin_chi"
    Private Const FieldDenDiem As String = "Den_tin_chi"
    Private Const HeadMa As String = "Xếp hạng"
    Private Const HeadMaEn As String = "Tên tiếng anh"
    Private Const HeadTuDiem As String = "Từ tín chỉ "
    Private Const HeadDenDiem As String = "Đến tín chỉ "
...
#Region "User Functions"
    ...
    Private Function ValidForm() As Boolean
        ...
        If txtDenDiem.Text = "" Then
            ...
            ValidForm = False
        End If
        If txtMa.Text.Length > 3 Then
            Me.ErrorProvider1.SetError(txtMa, "Không được nhập quá " & 3 & " ký tự!")
            txtMa.Focus()
            ValidForm = False
        End If
        If txtTuDiem.Text.Length > 3 Then
            Me.ErrorProvider1.SetError(txtTuDiem, "Không được nhập quá " & 3 & " ký tự!")
            txtTuDiem.Focus()
            ValidForm = False
        End If
        If txtDenDiem.Text.Length > 3 Then
            Me.ErrorProvider1.SetError(txtDenDiem, "Không được nhập quá " & 3 & " ký tự!")
            txtDenDiem.Focus()
            ValidForm = False
        End If
        If txtMa.Text.Length > 0 AndAlso Not IsNumeric(txtMa.Text) Then
            Me.ErrorProvider1.SetError(txtMa, "Phải nhập số!")
            txtMa.Focus()
            ValidForm = False
        End If
    ...
    Private Function CheckSelectedGridView() As Boolean
        ...
        If Not rowCurr Is Nothing Then
            ...
            ValueMaEn = rowCurr.Cells(FieldMaEn).Value.ToString
            ValueTuDiem = rowCurr.Cells(FieldTuDiem).Value.ToString
            ValueDenDiem = rowCurr.Cells(FieldDenDiem).Value.ToString
            SetValueFromUI()
            Return True
        Else
