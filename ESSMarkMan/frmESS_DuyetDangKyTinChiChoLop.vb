...
Imports ESS.Objects
Imports ESSReports
Public Class frmESS_DuyetDangKyTinChiChoLop
    ...
    Private Sub frmESS_DangKyLopTinChi_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            Dim str As String = "SELECT DISTINCT ID_lop FROM SYS_NguoiDungCoVanHocTap a INNER JOIN STU_HOSOSINHVIEN b ON a.Ma_sv=b.Ma_sv " & _
                                "INNER JOIN STU_DANHSACH c ON b.ID_SV=c.ID_SV WHERE UserName=N'" & gobjUser.UserName.Trim & "'"
            If gobjUser.ID_phong = 0 And gobjUser.UserName.ToUpper <> "ADMIN" Then
                Me.TreeViewLop_ChuanHoa1.HienThi_ESSTreeView_TheoCoVanHocTap(str)
            Else
    ...
    Private Sub TreeViewLop_ChuanHoa1_TreeNodeSelected_() Handles TreeViewLop_ChuanHoa1.TreeNodeSelected_, cmbTrang_thai_duyet.SelectedIndexChanged
        ...
        Try
            If Not TreeViewLop_ChuanHoa1.ID_lop_list Is Nothing AndAlso EssToolBarTC1.cmbID_chuyen_nganh.SelectedIndex <= 0 Then
                ...
                objDanhSach = New DanhSachSinhVien_Bussines(ID_lops, "SELECT ID_SV FROM STU_DANHSACH WHERE Trang_thai<>0")
                ...
                If cmbTrang_thai_duyet.Text.Trim <> "" Then
                    ...
                    If cmbTrang_thai_duyet.Text = "Đã duyệt" Then
                        sql = "SELECT DISTINCT a.ID_SV FROM STU_DANHSACHLOPTINCHI a INNER JOIN PLAN_LOPTINCHI_TC b ON a.ID_lop_tc=b.ID_lop_tc INNER JOIN PLAN_MONtINCHI_TC c ON b.ID_mon_tc=c.ID_mon_tc INNER JOIN STU_DANHSACH d ON a.ID_SV=d.ID_SV WHERE  Rut_bot_hoc_phan=0 AND Huy_dang_ky=0 AND Ky_dang_ky=" & EssToolBarTC1.Ky_dang_ky & " AND ID_lop in (" & ID_lops & ") AND Duyet=1"
                    Else
                        sql = "SELECT DISTINCT a.ID_SV FROM STU_DANHSACHLOPTINCHI a INNER JOIN PLAN_LOPTINCHI_TC b ON a.ID_lop_tc=b.ID_lop_tc INNER JOIN PLAN_MONtINCHI_TC c ON b.ID_mon_tc=c.ID_mon_tc INNER JOIN STU_DANHSACH d ON a.ID_SV=d.ID_SV WHERE Rut_bot_hoc_phan=0 AND Huy_dang_ky=0 AND Ky_dang_ky=" & EssToolBarTC1.Ky_dang_ky & " AND ID_lop in (" & ID_lops & ") AND Duyet=0"
                    End If
                    dt = clsDM.LoadDanhMuc(sql)

                    sql = "SELECT ID_SV FROM STU_HOSOSINHVIEN a " & _
                            "INNER JOIN SYS_NguoiDungCoVanHocTap b ON a.Ma_SV=b.Ma_SV " & _
                            "WHERE UserName = N'" & gobjUser.UserName & "'"
                    Dim dt_CVHT As DataTable = clsDM.LoadDanhMuc(sql)
                    dt_CVHT.DefaultView.Sort = "ID_SV"
                    If dt.Rows.Count > 0 Then
                        ...
                        For j As Integer = dv.Count - 1 To 0 Step -1
                            ...
                            Else
                                If gobjUser.ID_phong <= 0 Then
                                    dt_CVHT.DefaultView.RowFilter = "ID_SV=" & dv.Item(j)("ID_SV")
                                    If dt_CVHT.DefaultView.Count = 0 Then
                                        dv.Item(j).Delete()
                                    End If
                                End If
                            End If
                    ...
                    If cmbNgoai_ngu.Text <> "" Then
                        Ngoai_ngu = cmbNgoai_ngu.Text.Trim
                        ...
                        dv.RowFilter = " Ngoai_ngu ='" & Ngoai_ngu & "'"
                    End If
