    ...
    Private Sub trvLop_AfterSelect(ByVal sender As System.Object, ByVal e As System.Windows.Forms.TreeViewEventArgs) Handles trvPhongThi.AfterSelect
        ...
        If e.Node.Tag <> "" Then
            ...
            ElseIf Arr.Length >= 5 Then
                ...
                mNhom_tiet_phong = Arr(5)
                mCa_thi_Phong = Arr(6)
                mGio_thi_Phong = Arr(7)
                mNgay_thi_Phong = Arr(8)
            Else
                mID_khoa = 0
                mID_thi = 0
                mID_mon = 0
    ...
    Private Sub cmbHocky_Namhoc_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbHoc_ky.SelectedIndexChanged, cmbNam_hoc.SelectedIndexChanged
        If Loader Then
            ...
            If Hoc_ky > 0 And Nam_hoc <> "" Then
                Dim clsThi As TochucThi_Bussines
                If gobjUser.ID_phong > 0 Then
                    clsThi = New TochucThi_Bussines(Hoc_ky, Nam_hoc, 0, 0)
                Else
                    clsThi = New TochucThi_Bussines(Hoc_ky, Nam_hoc, gobjUser.ID_Bomon, gobjUser.ID_khoa)
                End If
                Dim dtThi As DataTable = clsThi.DanhSachMonToChucThi()
                TreeView_Lop(trvPhongThi, dtThi)
            End If
        End If
    End Sub
    Private Sub TreeView_Lop(ByVal trvThi As TreeView, ByVal dtThi As DataTable, Optional ByVal SelectAll As Boolean = False)
        trvThi.Nodes.Clear()
        Dim dr As DataRow
        Dim Nam_hoc As String = ""
        Dim ID_he As Long = -1, ID_khoa As Long = -1, ID_Mon As Long = 0, Dot_thi As Byte = 0, Lan_thi As Byte = 0, Tui_thi As Byte = 0, ID_phong_thi As Integer = 0
...
PhongThi:
                                trvThi.Nodes(i).Nodes(j).Nodes(k).Nodes(l).Nodes(m).Nodes.Add("Phòng thi: " + dr.Item("Ten_phong").ToString)
                                trvThi.Nodes(i).Nodes(j).Nodes(k).Nodes(l).Nodes(m).Nodes(n).Tag = dr.Item("ID_khoa").ToString + "#" + dr.Item("ID_thi").ToString + "#" + dr.Item("ID_mon").ToString + "#" + dr.Item("Lan_thi").ToString + "#" + dr.Item("ID_phong_thi").ToString + "#" + dr.Item("Nhom_tiet_Phong").ToString + "#" + dr.Item("Ca_thi_Phong").ToString + "#" + dr.Item("Gio_thi_Phong").ToString + "#" + dr.Item("Ngay_thi_Phong").ToString
                            ...
                            Else
                                ...
                                trvThi.Nodes(i).Nodes(j).Nodes(k).Nodes(l).Nodes(m).Tag = dr.Item("ID_khoa").ToString + "#" + dr.Item("ID_thi").ToString + "#" + dr.Item("ID_mon").ToString + "#" + dr.Item("Lan_thi").ToString
                    ...
                    Else
                        k += 1
                        trvThi.Nodes(i).Nodes(j).Nodes.Add("Học phần: " + dr.Item("Ten_mon").ToString + "-Mã HP: " + dr.Item("Ky_hieu").ToString)
            ...
            ID_phong_thi = dr.Item("ID_phong_thi")
        Next
    End Sub
#End Region
...
#Region "Property"
    ...
    Public ReadOnly Property ID_thi() As Integer
        ...
        End Get
    End Property
    Public ReadOnly Property ID_phong_thi() As String
        Get
    ...
    Public ReadOnly Property Gio_thi_Phong() As String
        Get
            Return mGio_thi_Phong
        End Get
    End Property
    Public ReadOnly Property Ngay_thi_Phong() As Date
        Get
            Return mNgay_thi_Phong
        End Get
    End Property
    Public ReadOnly Property Nhom_tiet_phong() As Integer
        Get
            Return mNhom_tiet_phong
        End Get
    End Property
    Public ReadOnly Property Ca_thi_Phong() As Integer
        Get
